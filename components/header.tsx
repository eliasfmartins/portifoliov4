...
const Header = () => {
  ...
  const MobileMenuItem = ({ item, index }: { item: any; index: number }) => {
    ...
    return (
      <motion.button
        onClick={handleClick}
        className={`relative w-full px-4 py-3 rounded-full transition-all duration-300 font-semibold font-rajdhani text-sm text-left ${
          activeSection === item.key
            ? "bg-gradient-to-r from-teal-400/50 to-cyan-400/50 text-white border-2 border-teal-400"
            : "bg-slate-800/90 text-white border border-teal-400/50 hover:bg-teal-400/20 hover:border-teal-400"
        }`}
  ...
  return (
    <>
              ...
              <div className="hidden lg:flex items-center space-x-1">
                {navItems.map((item, index) => (
                  <motion.button
                    ...
                    className={`relative px-3 lg:px-4 py-2 rounded-full transition-all duration-300 font-semibold font-rajdhani text-sm lg:text-base ${
                      activeSection === item.key
              ...
              <div className="flex items-center space-x-2 md:space-x-4">
                <motion.div whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }}>
                  <Button
                    ...
                    className="bg-slate-800/50 backdrop-blur-sm border border-teal-400/30 text-teal-400 hover:text-teal-300 hover:border-teal-400/50 hover:shadow-neon transition-all duration-300 text-xs md:text-sm px-2 md:px-3 py-1 md:py-2 font-semibold"
                  >
      ...
      {isScrolled && (
        ...
        >
          <div className="w-[90%] max-w-[1400px] bg-black/90 backdrop-blur-xl border border-teal-400/40 rounded-3xl px-6 py-4">
              ...
              <div className="hidden lg:flex items-center space-x-1">
                {navItems.map((item, index) => (
                  <motion.button
                    key={item.key}
                    onClick={() => scrollToSection(item.sectionId)}
                    className={`relative px-3 lg:px-4 py-2 rounded-full transition-all duration-300 font-semibold font-rajdhani text-sm ${
                      activeSection === item.key
                        ? "text-slate-900 font-semibold"
                        : "text-gray-300 hover:text-teal-400 hover:drop-shadow-neon"
                    }`}
              ...
              <div className="flex items-center space-x-2 md:space-x-4">
                <motion.div whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }}>
                  <Button
                    ...
                    size="sm"
                    onClick={() => setLanguage(language === "pt" ? "en" : "pt")}
                    className="bg-slate-800/50 backdrop-blur-sm border border-teal-400/30 text-teal-400 hover:text-teal-300 hover:border-teal-400/50 hover:shadow-neon transition-all duration-300 text-xs px-2 py-1 font-semibold"
                  >
          ...
          <AnimatePresence>
            {isMobileMenuOpen && (
              <motion.div
                className="mobile-menu-container lg:hidden absolute top-full left-1/2 transform -translate-x-1/2 mt-4 w-[90%] max-w-[1400px] bg-black/95 backdrop-blur-xl border-2 border-teal-400 rounded-2xl overflow-hidden z-50"
                initial={{ opacity: 0, height: 0 }}
                animate={{ opacity: 1, height: "auto" }}
                exit={{ opacity: 0, height: 0 }}
                transition={{ duration: 0.3 }}
