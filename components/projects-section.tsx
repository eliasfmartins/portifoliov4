				...
				>
					{filters.map((filter) => (
						<motion.button
							...
							className={`relative px-6 py-3 rounded-full font-semibold transition-all duration-400 overflow-hidden group ${activeFilter === filter.key
								...
								{filteredProjects.map((project, index) => {
									...
									return (
										...
										>
											...
											<div
												className={`relative w-72 h-44 rounded-xl overflow-hidden transform transition-all duration-500 ${selectedProject === index
				...
				<div className="flex justify-center space-x-3 overflow-x-auto pb-4">
					{filteredProjects.map((project, index) => (
						<button
							key={project.id}
							onClick={() => handleProjectSelect(index)}
							className={`relative flex-shrink-0 w-20 h-12 rounded-md overflow-hidden border-2 transition-all duration-300 group ${selectedProject === index
						...
						>
							...
							/>
							<div
								className={`absolute inset-0 transition-all duration-300 ${selectedProject === index ? "bg-teal-400/20" : "bg-black/40 hover:bg-black/20"
									}`}
							/>
							{/* Thumbnail indicator */}
							{selectedProject === index && (
